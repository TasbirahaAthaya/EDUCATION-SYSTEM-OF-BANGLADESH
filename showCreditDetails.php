...
if(isset($_POST['tc']))
{
    $SQL = oci_parse($conn,"select sum(credit) as total_credit from course
where crs_id=any
(select crs_id from teaches natural join employee
where emp_user_name='$id')");
}

// Descending Order
elseif (isset ($_POST['rc'])) 
    {
        $SQL = oci_parse($conn,"select (15-sum(credit))as Remaining_credit from course
where crs_id=any
(select crs_id from teaches natural join employee
where emp_user_name='$id')");
    }
