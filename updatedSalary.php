...
$SAL=$_POST['salary'];
$id=$_POST['i'];
$query1="UPDATE EMPLOYEE SET EMP_SALARY='$SAL' WHERE EMP_ID='$id'";
  
   $insert = oci_parse($conn,$query1);
   
    oci_execute($insert);

$SQL = oci_parse($conn,"SELECT EMP_USER_NAME,EMP_SALARY AS UPDATED_SALARY FROM EMPLOYEE WHERE EMP_ID=$id");
oci_execute($SQL);
